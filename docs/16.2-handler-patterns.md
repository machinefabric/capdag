...
layout: doc
permalink: /16.2-handler-patterns/
---
# Handler Patterns
...
struct ExtractMetadataOp: Op, Sendable {
    ...
    func perform(dry: DryContext, wet: WetContext) async throws {
        ...
        let pdfData = try requireStream(streams, mediaUrn: "media:ext=pdf")
...
let prompt = require_stream_str(&streams, "media:enc=utf-8")?;

// Optional binary argument — returns None if missing
let image_data = find_stream(&streams, "media:ext=png;image");

// Optional text argument — returns None if missing
let model_override = find_stream_str(&streams, "media:model-spec;enc=utf-8");
```
...
```rust
let max_tokens: u32 = find_stream_str(&streams, "media:max-tokens;enc=utf-8;numeric")
    .and_then(|s| s.parse().ok())
    .unwrap_or(50);  // default if missing or unparseable

let temperature: f32 = find_stream_str(&streams, "media:temperature;enc=utf-8;numeric")
    .and_then(|s| s.parse().ok())
    .unwrap_or(0.7);
```

This follows the `json_value_to_bytes` convention (see [15.4-PLANNER.md](/docs/15.4-planner)): strings become raw UTF-8 bytes, numbers are string-encoded.
...
```rust
for media_urn in &["media:enc=utf-8", "media:rst;enc=utf-8", "media:log;enc=utf-8", "media:enc=utf-8;ext=md"] {
...
fn require_text_content(streams: &[(String, Vec<u8>)]) -> Result<&[u8], StreamError> {
    for urn in &["media:enc=utf-8", "media:rst;enc=utf-8", "media:log;enc=utf-8", "media:enc=utf-8;ext=md"] {
...
// let data = find_stream(&streams, "media:ext=pdf").unwrap_or_default();
```
