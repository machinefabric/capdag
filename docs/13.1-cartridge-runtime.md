...
When a REQ arrives, the runtime finds the handler by matching the request's cap URN against registered cap URNs using `is_dispatchable(provider, request)` (see [../07-DISPATCH.md](/docs/07-dispatch)). When multiple handlers match, the runtime ranks by specificity — preferring exact matches, then refinements (more specific than requested), then fallbacks (more generic).
