...
layout: doc
permalink: /17.1-task-integration/
---
# Task Integration
...
3. **Resolve argument values**: Reads `slot_values` and `cap_settings` from the task context. These are passed to the planner's slot resolution (see [15.4-PLANNER.md](/docs/15.4-planner)).
...
Source: machfab `ops_rs::cap_interpreter`.
