...
- **Head feeding is unchanged** where the runtime resolves the reference:
  ...
  unbounded input stream in-process. Only host-mediated capture (a
  sandboxed cartridge that cannot open the device) requires the engine to
  stream head input frames itself.
