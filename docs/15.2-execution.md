...
output has exactly one consumer, and the downstream group takes no extra args.
Within a segment, all cap invocations open up front and per-edge forwarding
tasks (`forward_frames`) stream frames from cap i to cap i+1 live — the
downstream cap receives its first item before the upstream cap has emitted its
last. Credit flows per hop (acquire from the downstream cap's window, grant
the upstream cap as consumed — L10/L14), and the head group's input feed runs
concurrently with terminal collection (L15). Fan-out, fan-in boundaries, and
extra-args groups fall back to the materializing path.

Forwarded STREAM_START frames are **relabeled to the downstream edge's
...
must not reach the consumer as the stream label. The plan already proved the
producer's output conforms to the edge's input; `forward_frames` re-asserts
that conformance and fails hard on a violation (a wiring/type bug must surface
at the hop, not as a missing-arg error inside the cartridge).
