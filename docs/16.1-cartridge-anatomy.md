...
layout: doc
permalink: /16.1-cartridge-anatomy/
---
# Cartridge Anatomy
...
Every manifest must include the identity cap — `identity_cap()` in Rust or the equivalent in Swift. This cap handles the identity verification step during handshake (see [12.3-HANDSHAKE.md](/docs/12.3-handshake)). `CartridgeRuntime::new()` panics if `CAP_IDENTITY` (`cap:effect=none`) is missing from the manifest.
...
```rust
let cap = Cap::new(
    CapUrn::from_string("cap:in=\"media:ext=pdf\";out=\"media:record\";extract;target=metadata").unwrap(),
...
```rust
let arg = CapArg::new(
    "media:ext=pdf",          // media URN identifying this argument
    true,                 // required
    vec![ArgSource::Stdin { stdin: "media:ext=pdf".to_string() }],
);
```
...
let thumb_urn = generate_thumbnail_urn("media:ext=pdf");   // thumbnail generation
let meta_urn  = extract_metadata_urn("media:ext=pdf");     // metadata extraction
let outline_urn = extract_outline_urn("media:ext=pdf");    // outline extraction
let disbind_urn = disbind_urn("media:ext=pdf");            // page extraction
```
...
1. **Discovery**: The engine's cartridge registry or dev_cartridges mapping identifies which cartridge binary provides a given cap. The cartridge registry is versioned: binaries bake a `CARTRIDGE_REGISTRY_VERSION` (currently `1`, manifest URL prefix `.../v1/manifest`), and cartridges resolve on disk at `{registry_slug}/v{cartridge_registry_version}/{channel}/{name}/{version}/cartridge.json`, with the version directory pinned to the host's baked version like the channel. The `registry_slug` is a path-safe transform of the registry URL's authority (host plus optional `:port`) — lowercased, non-`[a-z0-9.-]` characters replaced by `-`, null URL → `dev` — not a hash, so all versions of one registry share a slug. A registry is ingested only if its manifest's `fabricRegistryUrl` matches the client's baked fabric registry, keeping caps comparable across every registry a client uses (see [15.2-execution.md](/docs/15.2-execution)).
...
3. **Handshake**: HELLO exchange and identity verification (see [12.3-HANDSHAKE.md](/docs/12.3-handshake)).
