...
The meta map also carries limit proposals: `max_frame`, `max_chunk`, and `max_reorder_buffer`. Both sides negotiate limits by taking the minimum of the two proposals. It also carries the mandatory non-negative integer `handler_capacity`: `0` means unlimited (and is what engine endpoints declare); a positive cartridge value is the maximum number of requests it can execute concurrently. A missing, negative, or non-integer capacity is a protocol violation, not an invitation to infer a default. See [12.3-HANDSHAKE.md](/docs/12.3-handshake) for the full handshake sequence.
