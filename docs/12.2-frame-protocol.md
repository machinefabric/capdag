...
|-----|------|------|-------------|
| 0 | version | u8 | Protocol version. Always 3. |
...
The meta map also carries limit proposals: `max_frame`, `max_chunk`, and `max_reorder_buffer`. Both sides negotiate limits by taking the minimum of the two proposals. See [12.3-HANDSHAKE.md](/docs/12.3-handshake) for the full handshake sequence.
...
- `class` (text): The failure CLASS — whose problem the failure is, DECLARED at the error's definition site (see 17.2 and the repo-root `docs/failure-taxonomy.md`). One of `input` | `resource` | `environment` | `internal`. An emitter without a classified error writes `internal`; a receiver treats a missing or unknown token as `internal` (unclassified means "ours", never a guess).
- `message` (text): Human-readable error description.
- `arg_urn` (text, optional): The media URN of the ONE argument the failure is attributed to, DECLARED by the emit source when — and only when — the failure is about a single argument (a malformed prompt, an oversized image). When the source did not attribute, the key is ABSENT — never present with an empty string — and no receiver infers an attribution. Written by `Frame::err_classified`, read by `error_arg_urn()`.

ERR and END are mutually exclusive terminals. A request ends with exactly one of them.

Source: `frame.rs` (`Frame::err`, `Frame::err_classified`, `error_code`, `error_class`, `error_arg_urn`, `error_message`).
...
- `chunk_count` (key 15): The total number of CHUNK frames sent in this stream, by the sender's count. The receiver can verify it received all chunks. **Optional in v3**: a stream announced `unbounded` makes no length promise and its STREAM_END omits chunk_count (L16).
