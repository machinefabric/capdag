...
## Final Progress Rides in END (v3)
...
- **Do not emit a trailing 100% progress LOG.** Pre-v3 code that did so raced
  the END frame and could lose the 100% forever; the terminal metadata design
...
- END/ERR is the LAST flow frame of a request (L4). The cartridge writer
  ...
  the handler) is dropped and counted (`post_terminal`), never written. Drops
  are observable in the runtime's protocol stats — nothing is silent.
