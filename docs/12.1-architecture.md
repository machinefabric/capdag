...
1. **REQ frames**: The switch matches the request's cap URN against each master's capability set using `is_dispatchable` (see [../07-DISPATCH.md](/docs/07-dispatch)). When multiple masters can handle a cap, the switch picks the most specific match using the ranking function (see [../08-RANKING.md](/docs/08-ranking)). The switch assigns a routing ID (XID) to the request and records the mapping so that response frames can find their way back.
...
- **Handshake**: When a cartridge is first needed, the host spawns the process, performs the HELLO exchange to get the cartridge's manifest, and runs identity verification (see [12.3-HANDSHAKE.md](/docs/12.3-handshake)).
...
The host sends a REQ for `CAP_IDENTITY` (`cap:effect=none`) with a deterministic nonce — CBOR-encoded `Text("bifaci")`, a 7-byte value. The request follows the standard streaming pattern: REQ → STREAM_START → CHUNK(nonce) → STREAM_END → END. The cartridge's identity handler echoes the nonce back through the same streaming pattern.
...
The current protocol version is 4, set in the `PROTOCOL_VERSION` constant (`capdag/src/bifaci/frame.rs`). Every frame carries this in key 0, and the HELLO handshake enforces it — a HELLO declaring any other version is a hard handshake failure (no cross-version operation).

Version 3 added credit-based per-stream flow control (the Credit frame and the negotiated `initial_credit` limit — see [12.5-FLOW-CONTROL.md](/docs/12.5-flow-control)), unbounded streams, terminal metadata on END (the final progress rides in the terminal frame), unified per-request state with counted drops, and protocol stats snapshots.

Version 2 replaced the old single-response model (the removed `Res` frame type, discriminant 2 — permanently retired) with multiplexed streaming: one or more named streams (STREAM_START/CHUNK/STREAM_END) terminated by END, chunk_index/chunk_count/checksum integrity fields, and routing_id for relay-aware routing.
