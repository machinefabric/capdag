...
The current protocol version is 3, set in the `PROTOCOL_VERSION` constant (`capdag/src/bifaci/frame.rs`). Every frame carries this in key 0, and the HELLO handshake enforces it — a HELLO declaring any other version is a hard handshake failure (no cross-version operation).
