...
An `InputStream` represents a single named input stream arriving from the host. Each stream carries a `media_urn` identifying the data type (e.g., `"media:pdf"`, `"media:enc=utf-8"`) and yields decoded CBOR values extracted from CHUNK frames.
...
// streams = [("media:pdf", <bytes>), ("media:enc=utf-8", <bytes>)]
```
