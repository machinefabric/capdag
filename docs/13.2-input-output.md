...
Resolution opens the device via the **built-in capture dispatch**
(`capdag::capture::open` — a CLOSED, compile-time set of backends matched
by reference-URN family in plain code, exactly as file paths are read by
plain code; capture is transport, not a capability, and there is no
registry and nothing to register) and delivers an **unbounded sequence
stream** of items labeled with the arg's stdin content URN. Each item carries per-item metadata
...
The backends: `media:live;synthetic`, a deterministic clock source
(selector params: `items`, `interval_ms`, `item_bytes`, `ring`, plus
overrun injection) used by the shared test range and available as a
fixture feed in every build; and the hardware backends (microphone via
alsa/avfoundation, webcam via v4l2/avfoundation) behind capdag's `capture`
cargo feature — enabled by the capture-capable builds (the CLI, the
engine, audiocartridge, videocartridge). A reference outside the known
families — or a hardware family in a build without the feature — is a
hard, named error.

**Which runtime resolves is a deployment fact, not a protocol one.** The
cartridge runtime resolves when the consumer is an op (the linear-machine
case: the first consuming cap's process opens the device). The HOST
resolves when the cartridge cannot reach the device (sandboxing), and —
the same rule from the other side — when the host itself is the consumer:
a live source feeding a ForEach region directly is iterated by the
ENGINE, so the engine opens the feed via the same dispatch and dispatches
one body per delivered item while the capture runs. The engine registers
the tap so a run stop closes it (close-tap → the feed ends → in-flight
bodies drain → a stopped run is a valid run, 15.2 §Runs Stop).
