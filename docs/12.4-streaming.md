...
No flow frame may follow an END for the same request ID (L4). Runtimes enforce this at their write stage: a post-terminal flow frame is dropped and counted (reason `post_terminal`), never written. Terminal metadata — the final progress and message — rides IN the END frame itself (L5), so nothing important can race the terminal. Intermediaries forward the terminal before releasing routing state; frames arriving for released state are counted drops, never protocol errors (L6).
