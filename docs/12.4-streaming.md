...
- **Scalar mode** (`is_sequence = false`, `OutputStream::write`): every CHUNK payload is a **self-contained CBOR value** — raw bytes are split at `max_chunk` boundaries and each piece is re-wrapped as its own `Value::Bytes`. Per-frame decoding is always valid.
