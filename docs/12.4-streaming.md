...
This enables multi-argument capability invocations. For example, a cap that accepts both a PDF document and a prompt string receives them as two separate streams within the same request — one with `media_urn = "media:pdf"` and one with `media_urn = "media:textable"`. On the response side, a handler can emit multiple output streams too.
