---
title: "Peer Invocation"
layout: doc
permalink: /13.3-peer-invocation/
---
# Peer Invocation
...
let response = req.peer()
    .call_with_bytes("cap:in=...;out=...;download", &[
        ("media:model-spec;enc=utf-8", model_spec.as_bytes()),
    ])
    .await?;
...
let arg1 = call.arg("media:enc=utf-8");
...
**`collect_bytes()`** accumulates all `Data` items into a byte vector and fails if it encounters an unhandled `Log` item. Use `collect_bytes_forwarding()` or drain `recv()` when the peer can emit diagnostics.

**`collect_value()`** requires exactly one `Data` item and fails if any `Log` item is present. It never silently drops peer diagnostics.

**`collect_bytes_forwarding(output, base, weight)`** accumulates finite peer data while forwarding progress into the caller's progress range and preserving the source attribution of ordinary diagnostic frames.
