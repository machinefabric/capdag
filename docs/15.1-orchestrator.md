...
```
[media:pdf -> extract -> media:enc=utf-8 -> summarize -> media:enc=utf-8]
```

```mermaid
graph LR
    A["media:pdf"] -->|extract| B["media:enc=utf-8"]
...
3. **Validate media compatibility**: Check that each node's media URN is consistent across all edges connecting to it. A node that is both the output of one cap (producing `media:enc=utf-8`) and the input of another (expecting `media:pdf`) is a conflict.
