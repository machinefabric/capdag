...
```
[media:pdf -> extract -> media:textable -> summarize -> media:textable]
```

```mermaid
graph LR
    A["media:pdf"] -->|extract| B["media:textable"]
    B -->|summarize| C["media:textable"]
```

Fan-in (multiple sources into one step):

```
[media:image -> describe -> media:textable]
[media:textable -> describe]
```
...
3. **Validate media compatibility**: Check that each node's media URN is consistent across all edges connecting to it. A node that is both the output of one cap (producing `media:textable`) and the input of another (expecting `media:pdf`) is a conflict.
