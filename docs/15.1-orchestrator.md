...
```
[extract cap:in="media:ext=pdf";out="media:record";extract;target=metadata]
```

Wiring declares data flow between steps, with source and target as media URNs:

```
[media:ext=pdf -> extract -> media:record]
```

A linear chain:

```
[media:ext=pdf -> extract -> media:enc=utf-8 -> summarize -> media:enc=utf-8]
```

```mermaid
graph LR
    A["media:ext=pdf"] -->|extract| B["media:enc=utf-8"]
    B -->|summarize| C["media:enc=utf-8"]
```

Fan-in (multiple sources into one step):

```
[media:image -> describe -> media:enc=utf-8]
[media:enc=utf-8 -> describe]
```
...
3. **Validate media compatibility**: Check that each node's media URN is consistent across all edges connecting to it. A node that is both the output of one cap (producing `media:enc=utf-8`) and the input of another (expecting `media:ext=pdf`) is a conflict.
...
- **Resolver-level** (inside `Machine::from_string`): cycle detection via Kahn's algorithm, source-to-cap-arg matching, unknown cap rejection. These run per-strand during anchor realization (see [09-MACHINE-NOTATION](/docs/09-machine-notation) §7 and §12).
- **Orchestrator-level** (inside `parse_machine_to_cap_dag`): media-type compatibility and structure-mismatch checks (record vs opaque, from [11-MEDIA-URNS](/docs/11-media-urns)) at each node. These run after the resolved Machine is built and its edges are walked.
