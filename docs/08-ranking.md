...
Where `spec_C` is Cap URN specificity (see [03-SPECIFICITY](/docs/05-specificity)).
...
The `is_comparable` predicate from [02-PREDICATES](/docs/04-predicates) is sometimes used for **discovery** but NOT for ranking:
...
Ranking is policy, not semantics. The dispatch predicate defines validity; ranking determines selection among valid options. Once a sequence of caps has been resolved into a workflow, that workflow is encoded as a graph and serialized via [07-MACHINE-NOTATION](/docs/09-machine-notation).
