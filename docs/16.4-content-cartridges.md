...
```mermaid
graph LR
    PDF["media:ext=pdf"] -->|generate_thumbnail| IMG["media:image;png"]
    PDF -->|extract_metadata| REC["media:record"]
    PDF -->|extract_outline| JSON["media:json"]
    PDF -->|disbind| LIST["media:pdf;list"]

    TXT["media:enc=utf-8"] -->|generate_thumbnail| IMG2["media:image;png"]
    TXT -->|extract_metadata| REC2["media:record"]
    MD["media:md;enc=utf-8"] -->|extract_outline| JSON2["media:fmt=json;record"]
```
