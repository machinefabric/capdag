---
title: "Content Cartridges"
layout: doc
permalink: /16.4-content-cartridges/
---
# Content Cartridges
...
```mermaid
graph LR
    PDF["media:ext=pdf"] -->|generate_thumbnail| IMG["media:ext=png;image"]
    PDF -->|extract_metadata| REC["media:record"]
    PDF -->|extract_outline| JSON["media:json"]
    PDF -->|disbind| LIST["media:ext=pdf;list"]

    TXT["media:enc=utf-8"] -->|generate_thumbnail| IMG2["media:ext=png;image"]
    TXT -->|extract_metadata| REC2["media:record"]
    MD["media:enc=utf-8;ext=md"] -->|extract_outline| JSON2["media:fmt=json;record"]
```
...
Each function takes a media URN (e.g., `"media:ext=pdf"`) and returns the cap URN string for that operation on that media type. The cap URN includes the appropriate `in=` and `out=` specs.
...
- `media:ext=pdf` (required, stdin): The PDF document.
- `media:width;enc=utf-8;numeric` (optional): Output width in pixels.
- `media:height;enc=utf-8;numeric` (optional): Output height in pixels.
- `media:index-range;enc=utf-8` (optional): Which pages to render (e.g., "0-2" for first three pages).
...
Arguments: `media:ext=pdf` (required, stdin).

### extract_outline

Extracts the PDF outline (bookmarks/table of contents) as a hierarchical JSON structure. Arguments:

- `media:ext=pdf` (required, stdin).
- `media:max-depth;enc=utf-8;numeric` (optional): Maximum depth of outline traversal.
- `media:include-order-indexes;enc=utf-8` (optional): Whether to include ordering indexes.
...
Cap URN dispatch (see [../07-DISPATCH.md](/docs/07-dispatch)) ensures that the incoming request matches exactly one of the registered media types. The handler's fallback loop over URNs finds the matching stream.
...
2. **Define cap URNs**: Use standard helpers or build custom ones:
   ```rust
   let thumb_cap = make_cap(generate_thumbnail_urn("media:ext=epub"));
   let meta_cap = make_cap(extract_metadata_urn("media:ext=epub"));
   ```
...
4. **Implement handlers**: Each handler implements `Op<()>`. See [16.2-HANDLER-PATTERNS.md](/docs/16.2-handler-patterns) for the standard patterns.

5. **Register and run**:
   ```rust
   let mut runtime = CartridgeRuntime::with_manifest(manifest);
   runtime.register_op(&generate_thumbnail_urn("media:ext=epub"), || Box::new(ThumbnailOp));
   runtime.register_op(&extract_metadata_urn("media:ext=epub"), || Box::new(MetadataOp));
   runtime.run().await?;
   ```
