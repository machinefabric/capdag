...
**Unified request state (v3, L7).** The six parallel routing maps of v2
...
Terminal frames (END, ERR) trigger `terminate(key, kind)` — one atomic removal. A flow frame arriving for a released key is a **counted drop** (`no_route`), never a protocol error and never silent (L6/L8).
