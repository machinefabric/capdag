...
pub struct RelaySwitch {
    ...
    requests: RwLock<RequestTable>,            // unified per-request state (L7)
    drops: Arc<DropCounters>,                  // counted drops (L8)
    xid_counter: AtomicU64,
    live_cap_fab: RwLock<LiveCapFab>,
    cap_registry: Arc<FabricRegistry>,
    // ...
}
```

**Unified request state.** The former parallel routing maps
(`request_routing`, `peer_requests`, `peer_call_parents`, `origin_map`,
`external_response_channels`, `rid_to_xid`) are one `RequestTable`
(`capdag/src/bifaci/request_state.rs`): one entry per in-flight request holding
its routing, origin, peer marker, cancel-cascade children, response channel,
per-stream flow stats, and timestamps, plus the rid→xid secondary index. A
request is **registered once** (duplicate registration is a protocol violation
and fails hard) and **terminated once** (`terminate(key, kind)` with kind
End | Err | Cancelled | MasterDied) — after termination, zero state remains for
the key, atomically. The terminal frame is forwarded using the state `terminate`
hands back, so state release and terminal delivery cannot disagree (L6).
...
3. Run identity verification: send REQ for `CAP_IDENTITY` (`cap:effect=none`) with nonce, verify echo response.
...
2. **Match** against each master's capability set using `is_dispatchable(candidate, request)` (see [../07-DISPATCH.md](/docs/07-dispatch)). The candidate's cap URN must accept the request's input type (contravariant) and produce a compatible output type (covariant).
3. **Rank** if multiple masters match. Specificity ranking (see [../08-RANKING.md](/docs/08-ranking)) selects the most specific candidate.
...
Terminal frames (END, ERR) trigger `terminate(key, kind)` — one atomic removal. A frame arriving with no routing state is **counted and classified for what it actually is** — never a protocol error and never silent (L6/L8): if its RID is in the table's bounded recently-terminated ring (`recently_terminated_rid`), the frame crossed its request's terminal in flight — a BENIGN post-terminal straggler, the ordinary teardown race of credit-based flow control, counted per frame type in the straggler counters and indicated as benign (it is NOT a drop; nothing went wrong). A RID the table never knew counts as a `no_route` DROP, the genuine routing-anomaly alarm. This classification (`account_unrouted_frame`) applies at all three unroutable sites: a duplicate terminal for a released request, a response frame with no routing state, and a continuation with no routing state.

Registration seeds each request's flow ledger with the **destination master's negotiated `initial_credit`** (`master_initial_credit`), and the engine-facing send path (`send_to_master`) records outbound continuation/control frames in the same ledger — per-stream `credit_outstanding` in the stats snapshot is therefore the stream's remaining window (see [12.5 — Flow Control](/docs/12.5-flow-control)).
...
These methods are used by the planner (see [15.4-PLANNER.md](/docs/15.4-planner)) to find execution paths through the capability graph.

Source: `relay_switch.rs`.

## Protocol Stats

`RelaySwitch::protocol_stats()` returns a serializable snapshot: every live
request's phase (created/streaming), age, idle time, per-stream frame/byte/
chunk/credit counters, and children; a bounded ring of recently-terminated
request summaries (kind, lifetime, flow totals); lifetime registration and
per-kind termination totals; and the per-reason drop counters. The host
runtime exposes the same shape via `protocol_stats()` (drop counters + routing
table sizes + GC totals), shipped to the switch inside every RelayNotify stats
republish; cartridges report their drop totals on every heartbeat.

The switch RETAINS each master's advertised host stats (`hosts` map on the
snapshot, keyed by master id — TEST7091), and the host ingests each
cartridge's heartbeat `drops_total` into the inventory record's
`runtime_stats.protocol_drops_total` (absent until the first reading —
TEST7090). Nothing on the L8 surface is parsed-and-discarded: every reading a
party produces is observable end-to-end (engine gRPC `GetProtocolStats`, the
run's `flow_states`, and the cartridge inventory).
