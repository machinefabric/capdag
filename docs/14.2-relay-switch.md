...
2. **Match** against each master's capability set using `is_dispatchable(provider, request)` (see [../07-DISPATCH.md](/docs/07-dispatch)). The provider's cap URN must accept the request's input type (contravariant) and produce a compatible output type (covariant).
3. **Rank** if multiple masters match. Specificity ranking (see [../08-RANKING.md](/docs/08-ranking)) selects the most specific provider.
