...
Terminal frames (END, ERR) trigger `terminate(key, kind)` — one atomic removal. A frame arriving with no routing state is a **counted, classified drop** — never a protocol error and never silent (L6/L8): if its RID is in the table's bounded recently-terminated ring (`recently_terminated_rid`), the frame crossed its request's terminal in flight and counts as `post_terminal` (the ordinary teardown race of credit-based flow control); a RID the table never knew counts as `no_route`, the genuine routing-anomaly alarm. This classification applies at all three unroutable sites: a duplicate terminal for a released request, a response frame with no routing state, and a continuation with no routing state.
