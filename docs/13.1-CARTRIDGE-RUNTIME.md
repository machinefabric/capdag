...
The manifest must always include `identity_cap()` — the identity capability required for handshake verification (see [12.3-HANDSHAKE.md](12.3-HANDSHAKE.md)). `CartridgeRuntime::new()` panics if `CAP_IDENTITY` (`cap:effect=none`) is missing.
