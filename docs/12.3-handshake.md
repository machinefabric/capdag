...
| `initial_credit` | 32 | Initial per-stream flow-control window, in CHUNK frames (see [12.5-FLOW-CONTROL.md](/docs/12.5-flow-control)). |
...
**Version enforcement (v3, L1):** each side declares its protocol version in the HELLO (`meta["version"]` and frame key 0). A version other than 3 is a hard handshake failure with a descriptive error naming both versions. There is no cross-version operation and no fallback.
  ...
  "cap_groups": [
    {
      ...
      "caps": [
        {
          "urn": "cap:in=\"media:enc=utf-8\";out=\"media:enc=utf-8\";generate;model=gguf",
...
After the host receives the manifest, it validates that `CAP_IDENTITY` (`cap:effect=none`) is present. Every cartridge must declare the identity capability because the handshake verifies it immediately after the Hello exchange.
...
The verifier MUST tolerate interleaved control and side-channel frames — **CREDIT**, **HEARTBEAT**, and **LOG** — while reading the echo. These are legal anywhere in a frame sequence (12.4: LOG interleaves without affecting data flow; CREDIT/HEARTBEAT are the control plane the writer's terminal gate itself exempts), and a v3 runtime that credits its probe input as the handler consumes it (L10) emits CREDIT frames during the exchange. Any other frame type is a hard verification failure.
