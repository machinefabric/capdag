...
**Version enforcement:** each side declares protocol version 4 in HELLO (`meta["version"]` and frame key 0). Any other version is a hard handshake failure with a descriptive error naming both versions. There is no cross-version operation and no fallback.
