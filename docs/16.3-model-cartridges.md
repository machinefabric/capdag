---
title: "Model Cartridges"
layout: doc
permalink: /16.3-model-cartridges/
---
# Model Cartridges
...
let response = req.peer().call_with_bytes(
    "cap:in=\"media:model-spec;enc=utf-8\";out=\"media:file-path;enc=utf-8\";download-model",
    &[("media:model-spec;enc=utf-8", &spec_bytes)],
).await?;
```
...
See [13.4-PROGRESS-AND-LOGGING.md](/docs/13.4-progress-and-logging) for the general progress forwarding pattern.
...
`run_with_keepalive` moves the blocking call to `tokio::task::spawn_blocking` (a separate thread pool) and emits progress frames every 30 seconds on the freed-up async runtime. See [13.4-PROGRESS-AND-LOGGING.md](/docs/13.4-progress-and-logging) for the mechanism.
...
Model specification arguments follow a standard convention: the media URN `media:model-spec;enc=utf-8` (with optional framework qualifier like `gguf`, `candle`, `mlx`) carries a HuggingFace repo ID or local path as UTF-8 text.
