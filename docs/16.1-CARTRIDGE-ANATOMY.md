...
Every manifest must include the identity cap — `identity_cap()` in Rust or the equivalent in Swift. This cap handles the identity verification step during handshake (see [12.3-HANDSHAKE.md](12.3-HANDSHAKE.md)). `CartridgeRuntime::new()` panics if `CAP_IDENTITY` (`cap:effect=none`) is missing from the manifest.
