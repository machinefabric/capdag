...
1. **Handler** returns `Err(OpError::Classified { code, class, message })` (or an unclassified `OpError::ExecutionFailed` — treated as `internal`).
2. **CartridgeRuntime** catches the error in `dispatch_op()`, preserving the declared code/class into `RuntimeError::Classified`.
3. **ERR frame** sent to stdout with the error code, class, and message (meta `code` + `class` + `message`).
...
6. **collect_terminal_output** receives the ERR frame and reads `error_code()` + `error_class()` structurally into `StreamIoError::Terminal { cap_urn, code, class, details }`.
7. Returns `ExecutionError::CartridgeExecutionFailed { cap_urn, code, class, details }`.
8. **cap_interpreter** records the failure with its classification; task state → `Failed` (or `TaskPermanentlyFailed` when `class == input`).
