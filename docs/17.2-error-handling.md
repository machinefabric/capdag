...
Admission is per cartridge identity and is driven by the mandatory
`handler_capacity` declared in HELLO and heartbeat state. Zero means unlimited;
for a positive value, work beyond available capacity waits in FIFO order.
Several process records carrying the same full install identity share that one
admission target; host registration order selects its authoritative capacity,
matching host-side request dispatch. Distinct install identities claiming the
same selected cap are an ambiguity and fail hard.

Positive-capacity caps are materialization boundaries in DAG execution. The
executor feeds and completes one such cap before admitting its dependent
segment; it never pre-acquires multiple bounded requests for a live pipeline.
The admission permit denotes an actively owned process slot, so pre-acquisition
could otherwise wait for a downstream slot while the upstream owner remains
unfed. Unlimited-capacity linear segments continue to stream live.
...
loss. The unavailable master rejects or releases its admission waiters; once a
replacement instance advertises capacity, subsequent queued work is admitted to
that live instance. This preserves body isolation in ForEach execution while
keeping process death visible as an `environment` failure on the affected body.
...
- One body failure does not terminate unrelated queued bodies.
