...
Every failure carries a CLASS — whose problem it is — DECLARED at the error's definition site and carried STRUCTURALLY through every layer above (the full architecture is `docs/failure-taxonomy.md` at the repo root):

| Class | Meaning | Retry |
|-------|---------|-------|
| `input` | Deterministic on the user's input (context overflow, unsupported format). | Never — permanent. |
| `resource` | A compute resource ran out (GPU VRAM, host memory). | Yes. |
| `environment` | The environment failed (network, deployment, model download, process death). | Yes. |
| `internal` | A defect in the engine or a cartridge. Ours. | Yes. |

The contract: a cartridge error enum declares `failure_class()` beside `error_code()`; the classified error crosses each boundary intact (`RuntimeError::Classified` → ERR frame meta `class` → `StreamIoError::Terminal` → `ExecutionError::CartridgeExecutionFailed` → the engine's run record). No layer ever infers another layer's class from message text — an error reaching a boundary without a declared class is `internal`.

### Argument Attribution

A classified failure MAY additionally name the ONE argument it is about: `arg_urn`, the media URN of the attributed argument. Attribution is first-class and OPTIONAL at every layer of the chain — the channel always exists (`OpError::Classified`/`WrappedClassified`, `RuntimeError::Classified`, ERR frame meta `arg_urn`, `StreamIoError::Terminal`, `ExecutionError::CartridgeExecutionFailed`, each with a `failure_arg_urn()`/`error_arg_urn()` accessor), but only the EMIT source decides whether a failure is about one argument:

- Attribution is declared at the source, exactly like class and code. A handler failing on a specific argument attaches its media URN (`RuntimeError::classified(...).with_arg_urn(...)`); wrapping layers carry it verbatim.
- Engine-originated argument-binding and validation failures that classify always attribute — a binding failure knows its arg by construction.
- Absence means "not about one argument". The ERR frame omits the `arg_urn` key entirely (never an empty string), every accessor returns `None`, and no layer ever infers an attribution downstream.
...
| `CartridgeExecutionFailed` | Cartridge returned ERR frame (code + class + details). |
...
| `FabricRegistryError` | Cartridge registry lookup or download failure. |
...
1. **Handler** returns `Err(OpError::Classified { code, class, message, arg_urn })` (or an unclassified `OpError::ExecutionFailed` — treated as `internal`). `arg_urn` is the handler's own attribution when the failure is about one argument, `None` otherwise.
2. **CartridgeRuntime** catches the error in `dispatch_op()`, preserving the declared code/class/arg_urn into `RuntimeError::Classified`.
3. **ERR frame** sent to stdout with the error code, class, and message (meta `code` + `class` + `message`, plus `arg_urn` when attributed).
...
6. **collect_terminal_output** receives the ERR frame and reads `error_code()` + `error_class()` + `error_arg_urn()` structurally into `StreamIoError::Terminal { cap_urn, code, class, details, arg_urn }`.
7. Returns `ExecutionError::CartridgeExecutionFailed { cap_urn, code, class, details, arg_urn }`.
8. **cap_interpreter** records the failure with its classification and attribution; task state → `Failed` (or `TaskPermanentlyFailed` when `class == input`).
...
Source: ggufcartridge `vision.rs`, `model.rs`. See [13.4-PROGRESS-AND-LOGGING.md](/docs/13.4-progress-and-logging).
...
Source: `cartridge_runtime.rs`. See [13.4-PROGRESS-AND-LOGGING.md](/docs/13.4-progress-and-logging).
...
- **Panics for invariant violations**: `Frame::req()` panics on invalid cap URNs. `CartridgeRuntime::new()` panics on missing `CAP_IDENTITY` (`cap:effect=none`). These are bugs in the calling code, not runtime errors.
